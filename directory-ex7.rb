...
def input_students
  ...
  puts "Please confirm that the cohort is #{cohort} with 'y' or re-enter it"
  confirm = gets.chomp
  if confirm == "y"
    cohort = cohort
  elsif confirm == ""
...
students << {name: name, cohort: cohort ||= "november".to_sym, hobby: hobby, bday: bday}
...
def print(names)
  ...
  puts "#{index+1}. #{student[:name]},".center(50) + "(#{student[:cohort]} cohort. Favorite hobby: #{student[:hobby]}, born #{student[:bday]})".center(10)
  end
end
